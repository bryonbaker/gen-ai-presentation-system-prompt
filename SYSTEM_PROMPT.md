...
1. `./FinOS/gen-ai-presentation-system-prompt/STYLE_GUIDE.md` — visual design rules, colors, fonts, and all available slide types
2. `./FinOS/gen-ai-presentation-system-prompt/presentation_helpers.py` — shared Python helper library (import this in your script)
3. `./FinOS/gen-ai-presentation-system-prompt/example_slides.py` — working code examples of every slide type

**Rules:**
- Use the template at `./FinOS/0 - clean template.pptx`
- Run all Python in the venv at `./FinOS/.venv` (has `python-pptx` installed)
- Import `presentation_helpers.py` using `sys.path.insert(0, './FinOS/gen-ai-presentation-system-prompt')` then `from presentation_helpers import *`
